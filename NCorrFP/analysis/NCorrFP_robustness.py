...
import utils
from NCorrFP_fidelity import hellinger_distance, kl_divergence_kde
...
def collusion(dataset='adult', save_results='robustness-collusion'):
    ...
    fp_params = {'gamma': [8],
                 'k': [300],
                 'fingerprint_length': [64, 128, 256, 512, 1024],
                 'n_recipients': [20],
                 'sk': [100]}
    collusion_params = {'n_colluders': [2, 3, 5, 10],
                        'strategy': ['avg', 'random', 'random_flip'],
                        'threshold': [1]}
    n_experiments = 10
        ...
        if os.path.isfile(file_path):
            ...
            for strength in attack_strength:
                for exp in range(n_experiments):
                    ...
                    for missing_col in missing_columns:  # TODO: record missing columns
...
def horizontal_attack(dataset='adult', save_results='robustness-horizontal'):
    ...
    results = {key: [] for key in ['gamma', 'k', 'fingerprint_length', 'n_recipients', 'sk', 'id', 'code',
                                   'embedding_ratio',
                                   'experiment_no', 'attack_strength', 'detection_confidence',
        ...
        if os.path.isfile(file_path):
            ...
            for strength in attack_strength:
                if buffer*strength <= 1:
                    cluster = pd.read_csv(
                        'NCorrFP/analysis/clusters/cluster_g1_k325_sk999.csv').head(int(buffer * strength * len(data.dataframe)))
                    cluster.index = cluster['Id']
                else:
                    cluster = pd.read_csv('NCorrFP/analysis/clusters/cluster_g1_k325_sk999.csv')
                    cluster.index = cluster['Id']
                for exp in range(n_experiments):
                    scheme = NCorrFP(gamma=param['gamma'], fingerprint_bit_length=param['fingerprint_length'],
                                     k=param['k'],
