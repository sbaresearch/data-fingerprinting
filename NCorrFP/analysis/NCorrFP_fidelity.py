...
def fidelity(dataset='covertype-sample', save_results='fidelity'):
    ...
    params = {'gamma': [2, 4, 8, 16, 32],
              'k': [300, 450],
              'fingerprint_length': [128, 256, 512],
              'n_recipients': [20],
    ...
    folder_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'fp_datasets', 'NCorrFP', dataset + '_fp')
    ...
    for combination in combinations:
        ...
        file_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'fp_datasets', 'NCorrFP', data.name + "_fp", file_name)
...
#            print(data.dataframe.columns); exit()
    ...
    if save_results is not None:
        results_univar_frame.to_csv(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'results', save_results), index=False)
        print(f"Results saved in {os.path.join(os.path.dirname(os.path.abspath(__file__)), 'results', save_results)}")
        results_bivar_frame.to_csv(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'results', save_bivar), index=False)
