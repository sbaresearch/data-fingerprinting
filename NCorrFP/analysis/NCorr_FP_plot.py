...
def plot_vote_error_rate(results, save=None):
    """
    ...
    for i, neighborhood_size in enumerate(sorted(grouped_data['k'].unique())):
        ...
        plt.plot(data['embedding_ratio'], data['mean'],
                 label=f'{neighborhood_size} / {round(100 * neighborhood_size / 32560, 1)}%',
                 marker=markers[i])
    ...
    # plt.ylim(0, 1)
    # Labels and legend
    plt.xlabel('Fingerprint embedding ratio (1/gamma) [0, 1]', fontsize=14)
    plt.ylabel('Vote Error Rate [0, 1]', fontsize=14)
    plt.xticks(fontsize=12)
    plt.yticks(fontsize=12)
    plt.title('Vote Error Rate vs Fingerprint embedding ratio with Neighborhood Size')
    plt.legend(title='Neighborhood Size\n(#records / %data size)', fontsize=14, title_fontsize=14)
    plt.grid(True)
    if save is not None:
...
def plot_tp_confidence(results):
    ...
    # Labels and legend
    plt.xlabel('Fingerprint embedding ratio (1/gamma) [0, 1]', fontsize=14)
    plt.ylabel('Correct extraction confidence [0, 1]', fontsize=14)
    plt.xticks(fontsize=12)
    plt.yticks(fontsize=12)
    plt.title('Correct extraction confidence')
    plt.legend(title='Fingerprint Length (in bits)', title_fontsize=14, fontsize=14)
    plt.grid(True)
    plt.show()
...
def plot_tp_tn_confidence(results, save=None):
    """
    ...
    markers = ['o', 'v', 'x', 's']
    legend_handles_DC = []
    legend_handles_DC.append(plt.Line2D([], [], linestyle='', label='Detection Confidence'))
    legend_handles_FAC = []
    legend_handles_FAC.append(plt.Line2D([], [], linestyle='', label='False Accusation Conf.'))
    ...
    for i, fingerprint_length in enumerate(grouped_data_tn['fingerprint_length'].unique()):
        ...
        # Plot for tn_hash
        line, = plt.plot(data_tn_hash['embedding_ratio'], data_tn_hash['mean'], label=f'{fingerprint_length}-bit (hash)', color=color, linestyle=line_styles[0], marker=markers[i])
        plt.fill_between(data_tn_hash['embedding_ratio'],
                         data_tn_hash['mean'] - data_tn_hash['std'],
                         data_tn_hash['mean'] + data_tn_hash['std'],
                         color=color, alpha=0.2)
        legend_handles_FAC.append(line)

        # Plot for tn_tardos
        line, = plt.plot(data_tn_tardos['embedding_ratio'], data_tn_tardos['mean'], label=f'{fingerprint_length}-bit (tardos)', color=color, linestyle=line_styles[1], marker=markers[i])
        ...
        # Plot for tp
        line, = plt.plot(data_tp['embedding_ratio'], data_tp['mean'], label=f'{fingerprint_length}-bit', color=color, linestyle=line_styles[2], marker=markers[i]) # linewidth=2
        plt.fill_between(data_tp['embedding_ratio'],
                         data_tp['mean'] - data_tp['std'],
                         data_tp['mean'] + data_tp['std'],
                         color=color, alpha=0.2)
        legend_handles_DC.append(line)

    # Labels and legend
    plt.xlabel('Fingerprint embedding ratio (1/$\gamma$) [0, 1]', fontsize=14)
    plt.ylabel('Detection confidence (DC) [0,1] \nFalse Accusation Confidence (FAC) [0,1]', fontsize=14)
#    plt.title('Extraction confidence')
    plt.xticks(fontsize=12)
    plt.yticks(fontsize=12)
    legend_handles_DC.extend(legend_handles_FAC)
    plt.legend(handles=legend_handles_DC, fontsize=14, title_fontsize=14, loc='lower right')#title='Fingerprint Length (in bits)')
    plt.grid(True)
    if save is not None:
        plt.savefig(save, dpi=300, bbox_inches='tight')
    plt.show()
