...
# todo: make compatible with older schemes
...
def init_balltrees(correlated_attributes, relation, dist_metric_discrete="hamming", dist_metric_continuous="minkowski",
                   categorical_attributes=None, show_messages=False):
    """
...
class NCorrFP():
    ...
    def create_fingerprint(self, recipient_id, secret_key, show_messages=False):
        """
        ...
        if recipient_id < 0 or recipient_id >= self.number_of_recipients:
            ...
            # todo: "You can change the n_recipients in config file."
            exit()
    ...
    def exact_matching_scores(self, fingerprint, secret_key):
        """
        Returns matching scores for each recipient and the detected fingerprint
    ...
    def insertion(self, dataset_name, recipient_id, secret_key, primary_key_name=None, outfile=None,
        ...
        if isinstance(dataset_name, datasets.Dataset):
            ...
            if correlated_attributes is None:
                correlated_attributes = utils.extract_mutually_correlated_groups(relation,
...
#        print(time_profile)
    ...
    def detection(self, dataset, secret_key, primary_key=None, correlated_attributes=None, original_columns=None,
        ...
        """
        print("Start NCorr fingerprint detection algorithm. This might take a few minutes ...")
        print("\tgamma: " + str(self.gamma) +
              "\n\tk: " + str(self.k) +
              "\n\tfp length: " + str(self.fingerprint_bit_length) +
              "\n\tmax # recipients: " + str(self.number_of_recipients))
        ...
        if original_columns is not None:  # aligning with original schema (in case of vertical attacks)
            if "Id" in original_columns:
                original_columns.remove("Id")  # why?
...
#            print(relation_fp.dataframe.columns)
        ...
        self.count = count
#        print(count)  # uncomment this to see the votes
...
#           print('Collusion scores: ', collusion_scores)
    ...
    def insertion_vpk(self, dataset_name, recipient_id, secret_key, outfile=None,
        ...
        tot_attributes = number_of_num_attributes + number_of_cat_attributes
        print("\tgamma: " + str(self.gamma) +
              "\n\tfingerprint bit-length: " + str(self.fingerprint_bit_length) +
              "\n\tk: " + str(self.k) +
