 ...
 #       print('Target values:' + str(list(self.relation.iloc[self.insertion_iter_log[iter]['neighbors']][self.insertion_iter_log[iter]['attribute']])))
    ...
    def show_detection_iteration(self, iteration, plotting_style='notebook', n_points=100, save_fig=None):
      ...
      #                       dense=mark_bit, plot=True, seed=self.insertion_iter_log[iter]['seed'])
            ...
            # Filter non-zero parts of masked PDF
            threshold_ins = min([mpdf for mpdf in masked_pdf_ins if mpdf > 0])
            threshold_det = min([mpdf for mpdf in masked_pdf_det if mpdf > 0])

#            x_ins_masked = [x for x, y in zip(x_ins, masked_pdf_ins) if y > threshold_ins]
#            x_det_masked = [x for x, y in zip(x_det, masked_pdf_det) if y > threshold_det]
...
#            axs[0].fill_between(x_ins, pdf_values_ins, 0, color='blue', alpha=0.1)
            # ############################### #
            # Code for splitting the hatching
            # Convert to numpy arrays for easier masking
            x_arr = np.array(x_ins)
            y_arr = np.array(pdf_values_ins)
            mask_arr = np.array(masked_pdf_ins)

            # Create a mask for where hatching should be applied
            hatch_mask = mask_arr <= -0.01
            # Get indices where mask is True
            hatch_indices = np.where(hatch_mask)[0]
            # Find contiguous True regions
            contiguous_regions = []
            current_region = []
            for i in range(len(hatch_mask)):
                if hatch_mask[i]:
                    ...
                    if current_region:
                        ...
                        current_region = []
            # Add the last region if it ends at the last index
            if current_region:
                contiguous_regions.append(current_region)
            # ############################## #
            for region in contiguous_regions:
                x_region = x_arr[region]
                y_region = y_arr[region]
                axs[0].fill_between(x_region, y_region, 0,
                                    facecolor='none',
                                    edgecolor='blue',
                                    hatch='\\',
                                    linewidth=0.0,
                                    zorder=0)
#            axs[0].fill_between(x_ins, pdf_values_ins, 0, facecolor='none', edgecolor='blue', hatch='\\', linewidth=0.0, zorder=0)
            axs[0].plot(x_ins, masked_pdf_ins, label='High density\n ({}th percentile)'.format(int(100 * 0.75)), linewidth=1.8)
#            axs[0].fill_between(x_ins, masked_pdf_ins, 0, color='orange', alpha=0.1)
            axs[0].fill_between(x_ins, masked_pdf_ins, 0, facecolor='none', edgecolor='orange', hatch='///', linewidth=0.0, zorder=0)
 ...
 #           axs[0].axhline(y=0.0001, color='black')
            ...
            axs[0].set_xlabel('Target values', fontsize=14)
 ...
 #           axs[0].set_title('Original data', fontsize=10)
            ...
            # Code for splitting the hatching
            # Convert to numpy arrays for easier masking
            x_arr = np.array(x_det)
            y_arr = np.array(pdf_values_det)
            mask_arr = np.array(masked_pdf_det)
            ...
            for region in contiguous_regions:
                x_region = x_arr[region]
                y_region = y_arr[region]
                axs[1].fill_between(x_region, y_region, 0,
                                    facecolor='none',
                                    edgecolor='blue',
                                    hatch='\\',
                                    linewidth=0.0,
            ...
            axs[1].plot(x_det, masked_pdf_det, label='High density\n ({}th percentile)'.format(int(100 * 0.75)),
                        linewidth=1.8)
            axs[1].fill_between(x_det, masked_pdf_det, 0, facecolor='none', edgecolor='orange', hatch='///', linewidth=0.0,
                        zorder=0)
            ...
            axs[1].set_xlabel('Target values', fontsize=14)
...
#            axes[0].set_title('Original data', fontsize=10)
            axes[0].set_xlabel('Target values', fontsize=14)
 ...
 #           axes[1].set_title('Fingerprinted data', fontsize=10)
            axes[1].set_xlabel('Target values', fontsize=14)
