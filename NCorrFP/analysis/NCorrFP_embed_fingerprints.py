    ...
    for combination in combinations:
        ...
        if not os.path.exists(file_path):
            ...
            scheme.insertion(data, secret_key=param['sk'], recipient_id=param['id'],
                             correlated_attributes=data.correlated_attributes, phi=0.75, save_computation=True,
                             outfile=file_path)
        else:
            print("- File already exists. {}".format(file_name))
    return os.path.join(os.path.dirname(os.path.abspath(__file__)), 'fp_datasets', 'NCorrFP', data.name + "_fp")
...
if __name__ == '__main__':
    ...
    params = {'gamma': [2, 4, 8, 16, 32],
              'k': [300, 450],
              'fingerprint_length': [128, 256, 512, 1024],
              'n_recipients': [20],
    ...
    # --- Embed fingerprints --- #
    dest = embed_fingerprints(data, params)
    print('Fingerprinted datasets written in: ', dest)
